...
EdgeList::deleteNet()
{
	if (net != NULL)
	{
		for (auto itr=net->begin(); itr!=net->end(); itr++)
		{
...
EdgeList::setUnion(vector<EdgeList*>* outnets)
{
	...
	for (int i=0;i<osize;i++)
	{
		...
		NET_T* onet = e->net;
		for (auto itr=onet->begin();itr!=onet->end();itr++)
		{
			string name = itr->first;
			map<string,double>* elist = itr->second;
			...
			for (auto gitr=elist->begin(); gitr!=elist->end(); gitr++)
			{
...
EdgeList::getOverlap(EdgeList* onet)
{
	...
	for (auto itr=oldNet->begin(); itr!=oldNet->end(); itr++)
	{
		string name = itr->first;
		if (net->find(name) == net->end())
		{
		...
		for (auto eitr=oldelist->begin(); eitr!=oldelist->end(); eitr++)
		{
			string gname = eitr->first;
