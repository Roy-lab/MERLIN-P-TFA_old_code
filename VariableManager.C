...
VariableManager::~VariableManager()
{
	for (auto itr=variableSet.begin();itr!=variableSet.end();itr++)
	{
...
{
	for (auto itr=varMngr->varNameIDMap.begin(); itr!=varMngr->varNameIDMap.end(); itr++)
	{
...
VariableManager::copyMe()
{
	...
	for (auto itr=varNameIDMap.begin(); itr!=varNameIDMap.end(); itr++)
	{
